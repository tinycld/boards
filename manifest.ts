const manifest = {
    ...
    // Boards is searchable through core's federated /api/search, which reads the
    ...
    quota: [{ collection: 'boards_attachments', sizeField: 'size', ownerField: 'uploaded_by' }],
    // Boards is rule-first: every authorization decision lives in the access
    ...
    server: { package: 'server', module: 'tinycld.org/packages/boards' },
    // `tinycld boards ...` commands, compiled into the per-org CLI binary by
