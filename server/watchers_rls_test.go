...
// cards_card_watchers access: any member may follow a card — for themselves
...
func TestWatchersRLS_ViewerCanWatchTheirOwnRow(t *testing.T) {
	...
	req{
		...
		url:     "/api/collections/cards_card_watchers/records",
...
func TestWatchersRLS_CanUnwatchOnlyYourself(t *testing.T) {
	...
	req{
		...
		url:    "/api/collections/cards_card_watchers/records/" + rows[env.viewer.Id],
		token:  env.editorToken,
...
func watcherRows(t *testing.T, env *cardsEnv) map[string]string {
	t.Helper()
	rows, err := env.app.FindRecordsByFilter("cards_card_watchers", "card = {:card}", "", 0, 0,
