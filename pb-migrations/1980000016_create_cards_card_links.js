...
//
// cards_card_links — one card blocks, duplicates or relates to another.
//
...
// hypothetical: cards_comment_reactions was left out of that endpoint's
...
// `@collection.cards_cards.project.cards_project_members_via_project`.
//
...
// `source.project.cards_project_members_via_project` and
...
// cards_cards' own rule, so reading a link discloses the far card's ID and
...
migrate(
    app => {
        const links = new Collection({
            id: 'pbc_cards_links_01',
            name: 'cards_card_links',
            type: 'base',
            system: false,
            fields: [
                {
                    id: 'cards_links_source',
                    name: 'source',
                    type: 'relation',
                    required: true,
            ...
            indexes: [
                'CREATE UNIQUE INDEX `idx_cards_links_unique` ON `cards_card_links` (`source`, `target`, `type`)',
                // Both directions are queried: the open card reads links where
                // it is either end.
                'CREATE INDEX `idx_cards_links_source` ON `cards_card_links` (`source`)',
                'CREATE INDEX `idx_cards_links_target` ON `cards_card_links` (`target`)',
            ],
        })
        app.save(links)
        ...
        const memberOf = ref =>
            `${ref}.project.cards_project_members_via_project.user ?= @request.auth.id`
        // The writing roles NAMED, never `?!= "viewer"` — trap 1 of
        // 1980000000, which is how drive silently granted commentor UPDATE.
        const writerOf = ref =>
            `${memberOf(ref)} && (${ref}.project.cards_project_members_via_project.role ?= "owner"` +
            ` || ${ref}.project.cards_project_members_via_project.role ?= "editor")`
        ...
        const tokenOn = (alias, ref) => {
            const c = `@collection.cards_share_links:${alias}`
            return (
        ...
        const col = app.findCollectionByNameOrId('cards_card_links')
    ...
    app => {
        ...
        app.delete(app.findCollectionByNameOrId('cards_card_links'))
    }
)
