...
//
// cards_activity — the per-card history: who changed what, and when.
//
...
migrate(
    app => {
        const activity = new Collection({
            id: 'pbc_cards_activity_01',
            name: 'cards_activity',
            type: 'base',
            system: false,
            fields: [
                {
                    id: 'cards_activity_project',
                    name: 'project',
                    type: 'relation',
                    required: true,
            ...
            indexes: [
                'CREATE INDEX `idx_cards_activity_card_created` ON `cards_activity` (`card`, `created`)',
                'CREATE INDEX `idx_cards_activity_project_created` ON `cards_activity` (`project`, `created`)',
            ],
        })
        app.save(activity)
        ...
        const col = app.findCollectionByNameOrId('cards_activity')
    ...
    app => {
        app.delete(app.findCollectionByNameOrId('cards_activity'))
    }
)
