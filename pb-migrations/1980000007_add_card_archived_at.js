...
//
// boards_cards.archived_at — when a card was archived, for the archived-items
...
//
// SERVER-OWNED, like boards_comments.edited_at: server/card_archived.go stamps
...
migrate(
    app => {
        ...
        app.db()
            .newQuery(
                "UPDATE boards_cards SET archived_at = updated WHERE archived = true AND archived_at = ''"
            )
            .execute()
    },
    app => {
        const cards = app.findCollectionByNameOrId('boards_cards')
        cards.fields.removeById('boards_cards_archived_at')
        app.save(cards)
    }
)
