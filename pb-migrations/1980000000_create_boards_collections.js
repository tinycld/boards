...
// Boards: kanban projects, lists, cards and their content.
//
// SCHEMA NOTES
//
...
// tinycld/boards/lib/rank.ts), not an integer: a move rewrites one row instead of
...
migrate(
    app => {
        ...
        const projects = new Collection({
            id: 'pbc_boards_projects_01',
            name: 'boards_projects',
            type: 'base',
            system: false,
            fields: [
                {
                    id: 'boards_projects_name',
                    name: 'name',
                    type: 'text',
                    required: true,
                    min: 1,
                ...
                {
                    id: 'boards_projects_archived',
                    name: 'archived',
                    type: 'bool',
                },
                {
                    id: 'boards_projects_created',
                    name: 'created',
                    type: 'autodate',
                    onCreate: true,
                    onUpdate: false,
            ...
            indexes: [
                'CREATE INDEX `idx_boards_projects_created_by` ON `boards_projects` (`created_by`)',
            ],
        })
        app.save(projects)

        // 2. boards_project_members
        const members = new Collection({
            id: 'pbc_boards_members_01',
            name: 'boards_project_members',
            type: 'base',
            system: false,
            fields: [
                {
                    id: 'boards_members_project',
                    name: 'project',
                    type: 'relation',
                    required: true,
            ...
            indexes: [
                ...
                'CREATE INDEX `idx_boards_members_project` ON `boards_project_members` (`project`)',
                'CREATE INDEX `idx_boards_members_user` ON `boards_project_members` (`user`)',
            ],
        })
        app.save(members)

        // 3. boards_share_links
        //
        ...
        //
        // A link grants access by MINTING a boards_project_members row when it is
        ...
        const shareLinks = new Collection({
            id: 'pbc_boards_share_links_01',
            name: 'boards_share_links',
            type: 'base',
            system: false,
            fields: [
                {
                    id: 'boards_sl_project',
                    name: 'project',
                    type: 'relation',
                    required: true,
                    collectionId: 'pbc_boards_projects_01',
                ...
                {
                    id: 'boards_sl_is_active',
                    name: 'is_active',
                    type: 'bool',
                },
                {
                    id: 'boards_sl_created',
                    name: 'created',
                    type: 'autodate',
                    onCreate: true,
                    onUpdate: false,
            ...
            indexes: [
                'CREATE UNIQUE INDEX `idx_boards_sl_token` ON `boards_share_links` (`token`)',
                'CREATE INDEX `idx_boards_sl_project` ON `boards_share_links` (`project`)',
            ],
        })
        app.save(shareLinks)

        // 4. boards_labels — before boards_cards, which relates to it
        const labels = new Collection({
            id: 'pbc_boards_labels_01',
            name: 'boards_labels',
            type: 'base',
            system: false,
            fields: [
                {
                    id: 'boards_labels_project',
                    name: 'project',
                    type: 'relation',
                    required: true,
                    collectionId: 'pbc_boards_projects_01',
            ...
            indexes: ['CREATE INDEX `idx_boards_labels_project` ON `boards_labels` (`project`)'],
        })
        app.save(labels)

        // 5. boards_lists
        const lists = new Collection({
            id: 'pbc_boards_lists_01',
            name: 'boards_lists',
            type: 'base',
            system: false,
            fields: [
                {
                    id: 'boards_lists_project',
                    name: 'project',
                    type: 'relation',
                    required: true,
                    collectionId: 'pbc_boards_projects_01',
                ...
                {
                    id: 'boards_lists_is_done',
                    name: 'is_done',
                    type: 'bool',
                },
                {
                    id: 'boards_lists_created',
                    name: 'created',
                    type: 'autodate',
                    onCreate: true,
                    onUpdate: false,
            ...
            indexes: [
                'CREATE INDEX `idx_boards_lists_project` ON `boards_lists` (`project`)',
                'CREATE INDEX `idx_boards_lists_project_position` ON `boards_lists` (`project`, `position`)',
            ],
        })
        app.save(lists)
        ...
        const cards = new Collection({
            id: 'pbc_boards_cards_01',
            name: 'boards_cards',
            type: 'base',
            system: false,
            fields: [
                {
                    id: 'boards_cards_project',
                    name: 'project',
                    type: 'relation',
                    required: true,
                    collectionId: 'pbc_boards_projects_01',
                ...
                {
                    id: 'boards_cards_list',
                    name: 'list',
                    type: 'relation',
                    required: true,
                    collectionId: 'pbc_boards_lists_01',
                    cascadeDelete: true,
                    maxSelect: 1,
                },
                {
                    id: 'boards_cards_position',
                    name: 'position',
                    type: 'text',
                    required: true,
                    min: 1,
                ...
                {
                    id: 'boards_cards_labels',
                    name: 'labels',
                    type: 'relation',
                    required: false,
                    collectionId: 'pbc_boards_labels_01',
                    cascadeDelete: false,
                    maxSelect: 20,
                },
                {
                    id: 'boards_cards_created_by',
                    name: 'created_by',
                    type: 'relation',
                    required: true,
                    collectionId: '_pb_users_auth_',
                ...
                {
                    id: 'boards_cards_archived',
                    name: 'archived',
                    type: 'bool',
                },
                // Board-face counters. boards_checklist_items and boards_comments
                ...
                // kanban scale.
                {
                    id: 'boards_cards_checklist_total',
                ...
                {
                    id: 'boards_cards_checklist_done',
                    name: 'checklist_done',
                    type: 'number',
                    required: false,
                    min: 0,
            ...
            indexes: [
                'CREATE INDEX `idx_boards_cards_list` ON `boards_cards` (`list`)',
                'CREATE INDEX `idx_boards_cards_list_position` ON `boards_cards` (`list`, `position`)',
                'CREATE INDEX `idx_boards_cards_project` ON `boards_cards` (`project`)',
                // Both due indexes serve the calendar event-source query, which
                // reads across a user's projects as well as within one.
                'CREATE INDEX `idx_boards_cards_due` ON `boards_cards` (`due`)',
                'CREATE INDEX `idx_boards_cards_project_due` ON `boards_cards` (`project`, `due`)',
            ],
        })
        app.save(cards)

        // 7. boards_checklist_items
        const checklistItems = new Collection({
            id: 'pbc_boards_checkitems_01',
            name: 'boards_checklist_items',
            type: 'base',
            system: false,
            fields: [
                {
                    id: 'boards_checkitems_card',
                    name: 'card',
                    type: 'relation',
                    required: true,
                ...
                {
                    id: 'boards_checkitems_is_done',
                    name: 'is_done',
                    type: 'bool',
                },
                {
                    id: 'boards_checkitems_position',
                    name: 'position',
                    type: 'text',
                    required: true,
                    min: 1,
            ...
            indexes: [
                'CREATE INDEX `idx_boards_checkitems_card` ON `boards_checklist_items` (`card`)',
                'CREATE INDEX `idx_boards_checkitems_card_position` ON `boards_checklist_items` (`card`, `position`)',
            ],
        })
        app.save(checklistItems)

        // 8. boards_comments — created WITHOUT `parent`; the self-relation needs
        // the collection to exist first (drive's drv_items_parent pattern).
        const comments = new Collection({
            id: 'pbc_boards_comments_01',
            name: 'boards_comments',
            type: 'base',
            system: false,
            fields: [
                {
                    id: 'boards_comments_card',
                    name: 'card',
                    type: 'relation',
                    required: true,
                    collectionId: 'pbc_boards_cards_01',
            ...
            indexes: ['CREATE INDEX `idx_boards_comments_card` ON `boards_comments` (`card`)'],
        })
        app.save(comments)

        // 8b. Add the self-relation now that boards_comments exists. `indexes` is
        // REPLACED wholesale rather than appended, so the card index is restated.
        const commentsCol = app.findCollectionByNameOrId('boards_comments')
        commentsCol.fields.add(
            new Field({
                id: 'boards_comments_parent',
                name: 'parent',
                type: 'relation',
                required: false,
        ...
        commentsCol.indexes = [
            'CREATE INDEX `idx_boards_comments_card` ON `boards_comments` (`card`)',
            'CREATE INDEX `idx_boards_comments_parent` ON `boards_comments` (`parent`)',
        ]
        app.save(commentsCol)

        // 9. boards_attachments
        //
        ...
        const attachments = new Collection({
            id: 'pbc_boards_attachments_01',
            name: 'boards_attachments',
            type: 'base',
            system: false,
            fields: [
                {
                    id: 'boards_attach_card',
                    name: 'card',
                    type: 'relation',
                    required: true,
                    collectionId: 'pbc_boards_cards_01',
            ...
            indexes: [
                'CREATE INDEX `idx_boards_attach_card` ON `boards_attachments` (`card`)',
                'CREATE INDEX `idx_boards_attach_uploaded_by` ON `boards_attachments` (`uploaded_by`)',
            ],
        })
        ...
        // On boards_projects itself — the back-relation starts at the project row.
        const isMember = 'boards_project_members_via_project.user ?= @request.auth.id'
        const isOwner = `${isMember} && boards_project_members_via_project.role ?= "owner"`
        ...
        // boards_projects: members read; any non-guest authed user creates a
        ...
        // boards_project_members: you always see your own row; the full roster is
        ...
        const bootstrapFirstOwner =
            ...
            ` && ${notGuest}`
        const membersCol = app.findCollectionByNameOrId('boards_project_members')
        setRules(membersCol, {
        ...
        // boards_share_links: owner-only in every direction. An editor or
        // commentor must not be able to mint a link that widens access.
        const shareLinksCol = app.findCollectionByNameOrId('boards_share_links')
        ...
        // boards_labels + boards_lists + boards_cards: members read, writers write.
        ...
        const labelsCol = app.findCollectionByNameOrId('boards_labels')
        setRules(labelsCol, {
        ...
        const listsCol = app.findCollectionByNameOrId('boards_lists')
        setRules(listsCol, {
            list: `${enabled} && ${viaMember}`,
            view: `${enabled} && ${viaMember}`,
            create: `${enabled} && ${viaWriter}`,
        ...
        // boards_cards.created_by is deliberately NOT pinned to the caller, and
        ...
        const checkItemsCol = app.findCollectionByNameOrId('boards_checklist_items')
        ...
        // boards_comments: a commentor may write one; the author is pinned to the
        ...
        const commentsRulesCol = app.findCollectionByNameOrId('boards_comments')
        ...
        // boards_attachments: the viewRule gates BOTH the record and the file
        ...
        // field in this workspace was — not boards', not mail's, not drive's,
        ...
        const attachmentsCol = app.findCollectionByNameOrId('boards_attachments')
