...
test.describe('Boards — priority', () => {
    ...
    test('sets a priority from the card, shows it on the face, and clears it', async ({ page }) => {
        ...
        await expect(face.locator('[data-testid^="boards-priority-"]')).toHaveCount(0)
        ...
        await expect(face.getByTestId('boards-priority-high')).toBeVisible()
    ...
    test('a priority survives leaving and reopening the board', async ({ page }) => {
        ...
        await expect(
            boardCard(page, CARD_TITLE).getByTestId('boards-priority-urgent')
        ).toBeVisible()
