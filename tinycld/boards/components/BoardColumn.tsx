...
export const BoardColumn = memo(function BoardColumn({
    ...
    const isCollapsed = useBoardsUIStore(s => !!s.collapsedColumnIds[list.id])
    ...
    const isSorted = useBoardsUIStore(s => selectBoardSort(s, projectId).field !== 'manual')
    // Same per-column discipline: `n` opens exactly one column's composer, and
    // only that column re-renders.
    const isComposerOpen = useBoardsUIStore(s => s.composerOpenListId === list.id)
    ...
    return (
        ...
        >
            ...
            >
                ...
                {isReceiving ? <View testID="boards-column-receiving" /> : null}
            </View>
    ...
    return (
        ...
        >
            <Pressable
                ...
                testID={`boards-column-collapsed-${list.id}`}
                onPress={onExpand}
...
function ColumnInsertionBar({ side, color }: { side: DropSide | null; color: string }) {
    ...
    return (
        <View
            testID="boards-column-insertion-bar"
            pointerEvents="none"
            style={{
                position: 'absolute',
                top: 6,
...
function ColumnDragHandle({
    ...
    return (
        <DraxView
            draggable={canDrag}
            dragPayload={{ kind: 'boards-column', listId: list.id }}
...
function EstimateTotal({ list }: { list: BoardListView }) {
    ...
    return (
        <View className="bg-foreground/[0.06] rounded-full px-1.5 py-px shrink-0">
            <Text
                className="text-[11px] font-semibold text-muted"
                testID={`boards-column-estimate-${list.id}`}
            >
            ...
            >
                {sortable.data.map((card, index) => (
                    <SortableItem
                        ...
                        sortable={sortable}
                        index={index}
                        payload={{ kind: 'boards-card', cardId: card.id, listId: list.id }}
