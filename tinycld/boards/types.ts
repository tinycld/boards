...
// before any typecheck, so `boards_*` is always present in pbSchema by the time
...
// not exist on type 'BoardsCards'" in a file you did not touch, pointing at a
...
import type {
    ...
    BoardsCards,
    BoardsCardWatchers,
    BoardsChecklistItems,
    BoardsCommentReactions,
    BoardsComments,
    BoardsEpics,
    BoardsLabels,
    BoardsLists,
 ...
 */
export type BoardsColor = string
 ...
 */
export type BoardsMemberRole = BoardsProjectMembers['role']

/** A share link may grant read, comment or edit — never ownership. */
export type BoardsShareLinkRole = BoardsShareLinks['role']
...
export type ActivityKind = BoardsActivity['kind']
...
export interface BoardCardView {
     ...
     *
     * The optionality lives HERE rather than on `BoardsCards.reporter`, which is
...
/**
 * An epic as the board renders it — the grouping row from boards_epics.
 *
 ...
 * `definePackageEntry<BoardsSchema>()`. It is an ecosystem contract, not a local
 ...
 * hand-written map had them as scalars. The alias is wider than boards' own
 * collections, which is harmless: the config intersects each package's schema
 * anyway, so extra members change nothing.
 */
export type { Schema as BoardsSchema } from '@tinycld/core/types/pbSchema'
