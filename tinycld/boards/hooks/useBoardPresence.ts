 ...
 */
export interface BoardsPresenceState {
    user: PresenceUser
    cardId: string | null
}

export interface RemoteCardsPresence extends BoardsPresenceState {
    clientID: number
}
 ...
 * an authorize handler and nothing else (boards/server/realtime.go).
 */
...
export function useRemoteCardsPresence(awareness: Awareness | null): RemoteCardsPresence[] {
    ...
    const entries = useRemoteAwareness<BoardsPresenceState>(awareness, options)
 ...
 */
export function parsePresence(raw: unknown): BoardsPresenceState | null {
    if (raw == null || typeof raw !== 'object') return null
    const obj = raw as Record<string, unknown>
 ...
 */
export function samePresence(a: BoardsPresenceState, b: BoardsPresenceState): boolean {
    return (
        a.user.id === b.user.id &&
