 ...
 *
 * Unlike `boards_labels` there is no unique index on the title: two epics called
...
export function useEpicMutations(projectId: string) {
    const [epicsCollection] = useStore('boards_epics')

    const createEpic = useMutation<string, Error, CreateEpicInput & { after: BoardEpic[] }>({
        mutationKey: ['boards', 'epic', 'create'],
    ...
    const updateEpic = useMutation<void, Error, UpdateEpicInput>({
        mutationKey: ['boards', 'epic', 'update'],
     ...
     *
     * `boards_cards.epic` is cascadeDelete FALSE (1980000017), so this leaves
    ...
    const deleteEpic = useMutation<void, Error, string>({
        mutationKey: ['boards', 'epic', 'delete'],
