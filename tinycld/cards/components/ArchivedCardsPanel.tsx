...
export function ArchivedCardsPanel({ project }: ArchivedCardsPanelProps) {
    const isOpen = useCardsUIStore(s => s.isArchivedPanelOpen)
    const close = useCardsUIStore(s => s.closeArchivedPanel)
...
function ArchivedCardsPanelBody({
    ...
    return (
        <Modal isOpen onClose={onClose}>
            <ModalBackdrop />
            <ModalContent testID="cards-archived-panel" className="w-[440px] max-h-[70vh] p-0">
...
function ArchivedRow({ row, canEdit }: { row: ArchivedCardRow; canEdit: boolean }) {
    ...
    return (
        <View
            testID={`cards-archived-row-${row.id}`}
