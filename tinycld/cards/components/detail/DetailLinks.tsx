...
function LinkRow({
    ...
    return (
        <View className="flex-row items-center gap-2 py-1.5" testID="cards-link-row">
            {link.far.state === 'redacted' ? <RedactedFar /> : <ResolvedFar link={link} />}
            {canEdit ? (
                <Pressable
                    accessibilityRole="button"
                    accessibilityLabel="Remove link"
                    testID="cards-link-remove"
                    onPress={onRemove}
...
function RedactedFar() {
    ...
    return (
        <View className="flex-row items-center gap-2 flex-1" testID="cards-link-redacted">
