 ...
 *
 * READS go through the collection like everything else — cards_share_links is
 ...
 * also flips `cards_projects.visibility`, which a client insert cannot do
...
export function useCreateShareLink(projectId: string) {
    ...
    return useMutation<ShareLinkRow, Error, CreateShareLinkInput>({
        mutationKey: ['cards', 'share-link', 'create', projectId],
        mutationFn: async ({ role, expiresInDays }: CreateShareLinkInput) => {
            try {
                const payload = await pb.send<ShareLinkPayload>('/api/cards/share-link', {
                    method: 'POST',
                    body: {
                ...
                captureException('cards.shareLink.create', err, { projectId, role })
                throw err
            }
        },
        ...
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['cards_share_links'] }),
    })
}
...
export function useRevokeShareLink(projectId: string) {
    ...
    return useMutation<void, Error, string>({
        mutationKey: ['cards', 'share-link', 'revoke', projectId],
        mutationFn: async (linkId: string) => {
            try {
                await pb.send(`/api/cards/share-link/${linkId}`, { method: 'DELETE' })
            } catch (err) {
                captureException('cards.shareLink.revoke', err, { projectId, linkId })
                throw err
            }
        },
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['cards_share_links'] }),
