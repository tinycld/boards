...
export function useMembershipVisibilitySync() {
    ...
    useEffect(() => {
        ...
        for (const projectId of granted) {
            ...
            Promise.all([
                pb.collection('cards_projects').getOne<CardsProjects>(projectId),
                pb
                    .collection('cards_project_members')
                    .getFullList<CardsProjectMembers & { expand?: { user?: Users } }>({
                        filter,
                        expand: 'user',
                    }),
                pb.collection('cards_lists').getFullList<CardsLists>({ filter }),
                pb.collection('cards_cards').getFullList<CardsCards>({ filter }),
                pb.collection('cards_labels').getFullList<CardsLabels>({ filter }),
            ])
                ...
                .catch(err => {
                    ...
                    captureException('cards.membershipVisibility.pull', err, { projectId })
                })
        }
