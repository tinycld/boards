 ...
 *
 * Cards does NOT use core's label system (`labels` + `label_assignments`),
 ...
 * team, which is why `cards_labels` is project-scoped and assignment is a
...
export function useLabelMutations(projectId: string) {
    const [labelsCollection] = useStore('cards_labels')

    const createLabel = useMutation<string, Error, CreateLabelInput>({
        mutationKey: ['cards', 'label', 'create'],
    ...
    const updateLabel = useMutation<void, Error, UpdateLabelInput>({
        mutationKey: ['cards', 'label', 'update'],
     ...
     *
     * `cards_cards.labels` is a relation with cascadeDelete FALSE, so deleting
    ...
    const deleteLabel = useMutation<void, Error, string>({
        mutationKey: ['cards', 'label', 'delete'],
